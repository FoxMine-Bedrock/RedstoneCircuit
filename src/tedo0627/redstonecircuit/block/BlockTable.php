...
class BlockTable {
    ...
    public function __construct() {
	    ...
	    foreach (RuntimeBlockMapping::PATHS as $protocol => $path) {
			$protocol = RuntimeBlockMapping::getMappingProtocol($protocol);

		    $legacyStateMapReader = PacketSerializer::decoder(
			    Utils::assumeNotFalse(file_get_contents(Path::join(BEDROCK_DATA_PATH, "r12_to_current_block_map". $path[1] .".bin")), "Missing required resource file"),
			    0,
			    new PacketSerializerContext(GlobalItemTypeDictionary::getInstance()->getDictionary(GlobalItemTypeDictionary::getDictionaryProtocol($protocol)))
		    );
			$legacyStateMapReader->setProtocolId($protocol);
		    ...
		    while (!$legacyStateMapReader->feof()) {
			    ...
			    $this->idToDamageToState[$protocol][$id][$damage] = $states;
			    $this->idToStateToDamage[$protocol][$id][$strStates] = $damage;
		    }
		    ...
		    foreach ($mapping->getBedrockKnownStates(RuntimeBlockMapping::getMappingProtocol($protocol)) as $tag) {
			    ...
			    $states = $tag->getCompoundTag("states");
			    $strStates = ltrim($states->toString(), "TAG_Compound=");
			    if (array_key_exists($id, $this->idToStateToDamage[$protocol])&&array_key_exists($strStates, $this->idToStateToDamage[$protocol][$id])) continue;
    ...
    public function getStates(int $id, int $damage, int $protocol = ProtocolInfo::CURRENT_PROTOCOL): CompoundTag {
        return $this->idToDamageToState[$protocol][$id][$damage];
    }

    public function getDamage(int $id, CompoundTag $states, int $protocol = ProtocolInfo::CURRENT_PROTOCOL): int {
        $strStates = ltrim($states->toString(), "TAG_Compound=");
        return $this->idToStateToDamage[$protocol][$id][$strStates];
    }
}