...
class RedstoneCircuit extends PluginBase {
	...
	public static function registerMappings(): void {
		foreach (ProtocolInfo::ACCEPTED_PROTOCOL as $protocol) {
			...
			$update = $mapping->toRuntimeId(Ids::INFO_UPDATE << Block::INTERNAL_METADATA_BITS, RuntimeBlockMapping::getMappingProtocol($protocol));
			...
			foreach ($mapping->getBedrockKnownStates(RuntimeBlockMapping::getMappingProtocol($protocol)) as $runtimeId => $tag) {
				$name = $tag->getString("name");
				if (!$table->existsId($name)) continue;

				$id = $table->getId($name);
				$states = $tag->getCompoundTag("states");
				$damage = $table->getDamage($id, $states, RuntimeBlockMapping::getMappingProtocol($protocol));
				if ($mapping->toRuntimeId(($id << Block::INTERNAL_METADATA_BITS) | $damage, RuntimeBlockMapping::getMappingProtocol($protocol)) !== $update) continue;

				$method->invoke($mapping, RuntimeBlockMapping::getMappingProtocol($protocol), $runtimeId, $id, $damage);
			}
		}
	}
